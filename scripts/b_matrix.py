...
def sphere_integral(p, q, s):
    ...
    if (p % 2 - 1)*(q % 2 - 1)*(s % 2 - 1) == -1:
        ...
        beta_2 = (1/2)*(q + 1)
        beta_3 = (1/2)*(s + 1)
        ret = 2 * gam_fun(beta_1) * gam_fun(beta_2) * gam_fun(beta_3) / gam_fun(beta_1 + beta_2 + beta_3)
...
def get_b_mat():
    ...
    # .X^2-Y^2
    grad_mat_2[4][4] = eig_2 * (sphere_integral(4, 0, 0) - 2 * sphere_integral(2, 2, 0) + sphere_integral(0, 4, 0))
    ...
    grad_mat_2[4][8] = eig_2 * (3 * (sphere_integral(2, 0, 2) - sphere_integral(0, 2, 2)) -
    ...
    # .3Z^2-1
    grad_mat_2[8][8] = eig_2 * (9 * sphere_integral(0, 0, 4) - 6 * sphere_integral(0, 0, 2) + sphere_integral(0, 0, 0))
