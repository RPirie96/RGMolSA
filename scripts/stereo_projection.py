...
def get_stereographic_projection(levels, inputs, rescaled, centres_r):
    ...
    alpha_mat = np.zeros((no_levels + 1, no_atoms), dtype=float)  # matrix of alpha coefficients
    beta_mat = np.zeros((no_levels + 1, no_atoms), dtype=complex)  # matrix of beta coefficients

    disc_radii = np.zeros((no_levels + 1, no_atoms), dtype=float)  # matrix of disc radii
    rel_radii = np.zeros((no_levels + 1, no_atoms), dtype=float)  # matrix of relative radii

    com_plan_cent_rel = np.zeros((no_levels + 1, no_atoms), dtype=complex)  # the relative centres
    com_plan_rad_rel = np.zeros((no_levels + 1, no_atoms), dtype=float)  # the relative radii
    ...
    for i in range(1, no_levels + 1):
        ...
        for k in range(0, no_atoms):
            if level_mat[i][k] > 0:
                sphere = k
                chain_s = get_chain(no_atoms, level_mat, adjacency_matrix, sphere, level)
                ...
                for q in range(1, level + 1):
                    ...
                    norm = math.sqrt((rel_cent[q][0] ** 2) + (rel_cent[q][1] ** 2) + (rel_cent[q][2] ** 2))
                    rel_cent[q] = rel_cent[q] / norm

                # Set up transformed vectors
                for q in range(2, level + 1):
                    rot_mat = get_m_rot([rel_cent[q - 1][0], rel_cent[q - 1][1], rel_cent[q - 1][2]])
                    inv_mat = la.inv(rot_mat)
                    for l in range(q, level + 1):
                        rel_vec = (np.dot(inv_mat, np.array([[rel_cent[l][0]], [rel_cent[l][1]],
                                                             [rel_cent[l][2]]]))).reshape(1, 3)
                        rel_cent[l] = rel_vec

                h_ght = 2 * radii_r[s_p_rel_mo] - abs(radii_r[s_p_rel_mo] - lam_r[s_p_rel_mo][s_p_rel])
                r_l = math.sqrt(h_ght / ((2 * radii_r[s_p_rel_mo]) - h_ght))

                h_ght_rel = 2 * radii_r[s_p_rel] - abs(radii_r[s_p_rel] - lam_r[s_p_rel][s_p_rel_mo])
                r_rel = math.sqrt(1 / (h_ght_rel / ((2 * radii_r[s_p_rel]) - h_ght_rel)))

                alpha_mat[i][k] = alpha_coefficient([rel_cent[level][0], rel_cent[level][1], rel_cent[level][2]])
                beta_mat[i][k] = beta_coefficient([rel_cent[level][0], rel_cent[level][1], rel_cent[level][2]])
                disc_radii[i][k] = r_l
                rel_radii[i][k] = r_rel
    ...
    for i in range(1, no_levels + 1):
        ...
        for k in range(0, no_atoms):
            if level_mat[i][k] > 0:
                ...
                com_plan_cent[i][k] = t_circle(alpha, beta, gamma, delta, com_plan_cent_rel[i][k],
                                               com_plan_rad_rel[i][k])[0]
                com_plan_rad[i][k] = t_circle(alpha, beta, gamma, delta, com_plan_cent_rel[i][k],
                                              com_plan_rad_rel[i][k])[1]
    ...
    for i in range(0, no_levels + 1):
        ...
        for k in range(0, no_atoms):
            if level_mat[i][k] > 0:
                ...
                d_0_mat_t[i][k] = 2 * (radii_r[k] ** 2) / ((abs(alpha) ** 2 + abs(gamma) ** 2) ** 2)
                d_1_mat_t[i][k] = -((np.conj(alpha) * beta) + (np.conj(gamma) * delta)) / (
                            abs(alpha) ** 2 + abs(gamma) ** 2)
                d_2_mat_t[i][k] = 1 / ((abs(alpha) ** 2 + abs(gamma) ** 2) ** 2)

    sgp = namedtuple('sgp', ['com_plan_cent', 'com_plan_rad', 'd_0_mat_t', 'd_1_mat_t', 'd_2_mat_t'])

    return sgp(com_plan_cent=com_plan_cent, com_plan_rad=com_plan_rad, d_0_mat_t=d_0_mat_t, d_1_mat_t=d_1_mat_t,
               d_2_mat_t=d_2_mat_t)
